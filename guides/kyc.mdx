...
<Tabs>
  ...
  POST /bank/add-account
  {
    ...
    "ifsc": "ABCD0001234", // Bank identifier code
...
<Note>
For Indian users, IFSC code is required. For international users, refer to [international bank requirements](/guides/international-banking).
</Note>
...
- Process payments using our [Payment API](/guides/payment)
...
## Best Practices for Smooth KYC

1. **Provide Clear Instructions to Users**
   - Explain what documents they need before starting
   - Give guidance on image quality requirements
   - Set expectations about the verification timeframe
...
3. **Secure Document Handling**
   - Never store sensitive document images on your servers
   - Use encrypted connections for all data transfers
   - Implement proper access controls for KYC data

4. **Optimize for Mobile Users**
   - Provide camera capture guidance for document photos
   - Design mobile-friendly KYC collection forms
   - Test the flow on multiple device types

<Tip>
Create a progress indicator for users showing where they are in the KYC process to improve user experience and reduce abandonment. Consider adding a status page where users can check their verification progress.
</Tip>
