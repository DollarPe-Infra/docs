...
flowchart TD
    ...
    CheckKYC -->|YES| CheckLimit["Check Payin Limit"]
    CheckKYC -->|NO| AddKYC["Complete KYC & Bank<br/>Verification"]

    AddKYC -->|API Call| KycAPI["/API: /kyc/add-kyc-data/"]
    KycAPI --> CheckKYC

    CheckLimit -->|Limit Available| FetchRate["Fetch Exchange Rate"]
    CheckLimit -->|EDD Required| EDDStep["Complete EDD<br/>(Enhanced Due Diligence)"]
    CheckLimit -->|Limit Exhausted &<br/>EDD Done| LimitError["Show Limit Exhausted/Error"]

    EDDStep -->|API Call| EddAPI["/API: /kyc/add-edd-data"]
    EddAPI --> CheckLimit
...
sequenceDiagram
    ...
    alt KYC & Bank Verified
        Exchange->>API: Check Payin Limit
        API->>Exchange: Return Limit Info
        alt Limit Available
            ...
            Exchange->>API: Complete EDD (Enhanced Due Diligence)
            API->>Exchange: EDD Result
            Exchange->>API: Check Payin Limit (again)
        else Limit Exhausted & EDD Done
            API->>Exchange: Show Limit Exhausted/Error (No further payins allowed)
        end
...
<Tabs>
  ...
  ```json
  {
    ...
    "message": "Success",
    "data": {
      "maximum_daily_limit": "100000.00",
      "available_daily_limit": "50000.00",
...
- `maximum_daily_limit`: The maximum daily payin limit for the customer
- `available_daily_limit`: The remaining payin limit for the day
- `is_edd_required`: If true, the customer must complete Enhanced Due Diligence (EDD) before proceeding

**How to use:**

- If `available_daily_limit` is sufficient for the intended transaction and `is_edd_required` is false, proceed to the next step.
